...
import { getToken, loadTokenStore, saveTokenStore, setToken } from './token-store.js';
 ...
 */
export async function refreshAccessToken(config: OAuthConfig, refreshToken: string): Promise<OAuthToken> {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      throw new OAuthError('refresh_failed', `Token refresh failed: HTTP ${response.status} ${text}`);
    }
...
export async function fetchClientCredentialsToken(config: OAuthConfig): Promise<OAuthToken> {
  ...
  try {
    ...
    if (!response.ok) {
      const text = await response.text().catch(() => '');
      throw new OAuthError('refresh_failed', `Client credentials token fetch failed: HTTP ${response.status} ${text}`);
    }

    return parseTokenResponse(await response.json() as Record<string, unknown>);
  } catch (error) {
    if (error instanceof OAuthError) throw error;
    throw new OAuthError('refresh_failed', `Client credentials token fetch failed: ${String(error)}`);
  }
}
...
export async function exchangeAuthorizationCode(
  ...
  try {
    ...
    if (!response.ok) {
      const text = await response.text().catch(() => '');
      throw new OAuthError('exchange_failed', `Authorization code exchange failed: HTTP ${response.status} ${text}`);
    }
...
export class TokenManager {
  ...
  async load(): Promise<void> {
    this.store = await loadTokenStore(this.authFilePath);
  }
  ...
  getStatus(providerName: string, config: OAuthConfig): AuthStatus {
    return classifyStatus(getToken(this.store, providerName), config);
  }
  ...
  async ensureValidToken(providerName: string, config: OAuthConfig): Promise<OAuthToken> {
    const token = getToken(this.store, providerName);
  ...
  async exchangeCode(
    ...
    this.store = setToken(this.store, providerName, token);
    await saveTokenStore(this.authFilePath, this.store);
    return token;
  }
