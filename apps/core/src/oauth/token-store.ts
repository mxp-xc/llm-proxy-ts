...
export function mergeTokenStore(data: AuthFileData, store: TokenStore): AuthFileData {
  ...
  for (const key of Object.keys(result)) {
    if (key === PLUGINS_KEY) continue;
    if (!(key in store) && typeof result[key] === 'object' && result[key] !== null && 'accessToken' in (result[key] as object)) {
      delete result[key];
    }
  }
  return result;
...
// ---- 兼容层：保留现有 TokenStore 级别 API ----

/**
 * 从 auth.json 文件加载 TokenStore。
 * 文件不存在返回空 store；JSON 损坏返回空 store 并记录警告。
 *
 * @deprecated 内部使用，新代码应使用 loadAuthFile + extractTokenStore
 */
export async function loadTokenStore(filePath: string): Promise<TokenStore> {
  const data = await loadAuthFile(filePath);
  return extractTokenStore(data);
}

/**
 * 原子写入 TokenStore 到 auth.json。
 * 保留 auth.json 中已有的 _plugins 等非 OAuth 数据。
 *
 * @deprecated 内部使用，新代码应使用 saveAuthFile + mergeTokenStore
 */
export async function saveTokenStore(filePath: string, store: TokenStore): Promise<void> {
  const data = await loadAuthFile(filePath);
  const merged = mergeTokenStore(data, store);
  await saveAuthFile(filePath, merged);
}
