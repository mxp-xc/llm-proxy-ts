...
function validatePluginShape(plugin: unknown, source: string): asserts plugin is Plugin {
  ...
  // 至少实现一个 hook
  const hookNames = [
    'init', 'beforeServerStart', 'afterServerStart',
    'beforeRequest', 'beforeProviderCall', 'afterProviderResult',
    'inspectStreamChunk', 'mapProviderError',
    'createFetch', 'discoverModels',
  ] as const;
