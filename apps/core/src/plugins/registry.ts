import type { Plugin, ProxyPlugin, AuthPlugin, PluginInitContext, ProviderContext, DiscoveredModelList, ResolvedPlugin } from './types.js';
...
export class PluginRegistry {
  ...
  static async fromSettings(
    ...
    for (const entry of settings.plugins) {
      ...
      log.info({ plugin: plugin.name, module: modulePath, providers: rp.providers }, 'global plugin loaded');
    ...
    for (const [providerId, provider] of Object.entries(settings.providers)) {
      ...
      for (const entry of provider.plugins) {
        ...
        log.info({ plugin: plugin.name, module: modulePath, provider: providerId }, 'provider plugin loaded');
      }
