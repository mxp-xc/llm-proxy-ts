...
function parseArgs(argv: string[]): { command: string; subcommand?: string; provider?: string; dryRun: boolean } {
  ...
  const result: { command: string; subcommand?: string; provider?: string; dryRun: boolean } = { command, dryRun };
...
async function main(): Promise<void> {
  ...
  if (command === 'models' && subcommand === 'sync') {
    ...
    if (!existsSync(settingsPath)) {
      ...
      console.error('Create one from the example: cp config/settings.example.jsonc config/settings.jsonc');
      process.exit(1);
    }
