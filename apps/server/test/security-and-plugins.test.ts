...
describe('vendor_sse_error', () => {
  ...
  it('does not call the gateway when a stream error is detected before sending headers', async () => {
    ...
    const response = await app.request('/v1/chat/completions', {
      ...
      body: JSON.stringify({ model: 'openrouter/chat', stream: true, messages: [{ role: 'user', content: 'hi' }] }),
    });
  ...
  it('emits a safe SSE error chunk when a later stream chunk contains a vendor error', async () => {
    ...
    expect(body).toContain('"error":{"message":"Rate limited by upstream provider","code":"rate_limit","type":"rate_limit_error"}');
...
describe('environment file loading', () => {
  it('loads root and app env files with local files overriding earlier values', async () => {
    ...
    await writeFile(join(rootDir, '.env'), 'ROOT_ONLY=root\nSHARED_VALUE=root\nLOCAL_VALUE=root-env\n');
    await writeFile(join(rootDir, '.env.local'), 'SHARED_VALUE=root-local\nLOCAL_VALUE=root-local\n');
...
async function* streamError(): AsyncIterable<unknown> {
  yield { type: 'raw', rawValue: 'data: {"error":{"message":"secret text","code":"rate_limit","type":"rate_limit_error"}}\n\n' };
}
