...
describe('OAuth callback', () => {
  ...
  describe('GET /oauth/callback', () => {
    it('exchanges authorization code for token', async () => {
      ...
      const state = Buffer.from(JSON.stringify({ provider: 'oauth-provider', nonce })).toString('base64url');
      const app = createOAuthCallbackApp(createDeps());
    ...
    it('rejects invalid state (wrong nonce)', async () => {
      const state = Buffer.from(JSON.stringify({ provider: 'oauth-provider', nonce: 'wrong-nonce' })).toString('base64url');
      const app = createOAuthCallbackApp(createDeps());
    ...
    it('handles OAuth error response', async () => {
      const app = createOAuthCallbackApp(createDeps());
      const res = await app.request('/callback?error=access_denied&error_description=User+cancelled');
