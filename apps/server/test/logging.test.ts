...
describe('logging', () => {
  it('writes plain text console logs with one newline per entry', async () => {
    ...
    try {
      ...
      logger.info({ method: 'GET', path: '/health', status: 200, durationMs: 3 }, 'request completed');
      ...
      expect(lines[0]).toMatch(/^\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2} INFO\s+llm-proxy - request completed method=GET path=\/health status=200 durationMs=3$/);
      expect(lines[1]).toMatch(/^\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2} WARN\s+llm-proxy - provider key selected provider=openrouter keyIndex=1 keyCount=3$/);
    } finally {
      stdout.restore();
    }
  });
...
function captureStdout(): { output: string; waitForWrites: (count: number) => Promise<void>; restore: () => void } {
