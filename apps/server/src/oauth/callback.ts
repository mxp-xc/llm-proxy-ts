...
export function createOAuthCallbackApp(deps: OAuthCallbackDeps): Hono {
  ...
  app.get('/login/:provider', (c) => {
    ...
    if (!provider?.oauth) {
      return c.json({ error: { type: 'invalid_request', message: `No OAuth configuration for provider '${providerName}'` } }, 404);
    }

    const oauth = provider.oauth;

    if (oauth.flow !== 'authorization_code') {
      return c.json({ error: { type: 'invalid_request', message: `Provider '${providerName}' does not use authorization_code flow` } }, 400);
    }

    if (!oauth.authorizationUrl) {
      return c.json({ error: { type: 'invalid_request', message: `Provider '${providerName}' missing authorizationUrl` } }, 400);
    }

    const redirectUri = oauth.redirectUri ?? `http://127.0.0.1:${settings.service.port}/oauth/callback`;
    const state = encodeState(providerName, nonce);
    const scope = oauth.scopes.length > 0 ? `&scope=${encodeURIComponent(oauth.scopes.join(' '))}` : '';
  ...
  app.get('/callback', async (c) => {
    ...
    const provider = settings.providers[providerName];
    if (!provider?.oauth) {
      return c.html(renderErrorPage('invalid_provider', `No OAuth configuration for provider '${providerName}'`));
    }
...
function decodeState(state: string): DecodedState | null {
  try {
    ...
    if (
      ...
      'provider' in parsed && 'nonce' in parsed &&
