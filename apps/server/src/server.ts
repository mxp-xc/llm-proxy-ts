...
import { loadSettingsFromFile, loadEnvironmentFiles, resolveSettingsPath, settingsSchema, TokenManager, PluginRegistry, createProviderRegistry } from '@llm-proxy/core';
...
async function main(): Promise<void> {
  ...
  // 加载插件
  const pluginRegistry = await PluginRegistry.fromSettings(settings, settingsDir, authFilePath, logger);
  ...
  const app = createApp({
    ...
    authFilePath,
    pluginRegistry,
    providerRegistry: await createProviderRegistry(settings, tokenManager, logger, pluginRegistry, authFilePath),
