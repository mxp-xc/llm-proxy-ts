...
**status 映射**：`mapResponseStatus` 将 finishReason 映射为 `completed` / `incomplete`。有 function / custom tool call 是正常完成（模型回合结束，等客户端执行工具），status=`completed`；仅 `length` / `content-filter`（真正未完成）为 `incomplete`。hosted `web_search_call`（`providerExecuted`，上游 inline 执行）同样 `completed`。
