...
**Goal:** 让 codex 经 llm-proxy 转发到 openai-compatible 上游（GLM）时，tool_search 发现的 namespace 工具（如 spawn_agent）能被 GLM 调用，并修复现有 mcp\_\_/codex_app namespace 命名回路。
...
### Task 6: mcp\_\_/codex_app 回归 + 端到端 + 全量验证

**Files:**

- Test: `test/providers/openai-responses/renderer.test.ts`（补 codex_app 响应侧拆回）
- Test: `test/providers/openai-responses/protocol.test.ts`（补 mcp\_\_ 请求侧历史 function_call 拍平 + 请求侧一致性）
- Test: `test/server/codex-endpoint.test.ts`（端到端，若存在；否则 Step 3 退而用 protocol 组合测试）
- Verify: 全量 typecheck + test

**Interfaces:**

- 验证 Task 1-5 覆盖现有 mcp\_\_/codex_app 路径（同一机制），handle-protocol 接线端到端通，不回归。
...
- [ ] **Step 2: 补 mcp\_\_ 请求侧历史 function_call 拍平测试**

在 `protocol.test.ts` 的 `describe('historical function_call namespace → flattened toolName', ...)` 内加（验证 mcp\_\_ 路径请求侧拍平，与 multi_agent_v1 同机制）：
...
it('request-side tools[] and namespaceFlatMap agree on flatName', () => {
  ...
  expect(flatMap!.get('multi_agent_v1__spawn_agent')).toEqual({
    namespace: 'multi_agent_v1',
    name: 'spawn_agent',
  })
})
...
- 现有 mcp\_\_/codex_app 路径修复 → Task 6（同一机制覆盖）✓
