...
- 每个模型:合并 4 层得到 `templateSlug`(全层未配时动态取 catalog 首个 `supported_in_api=true` 的 slug,无则 `gpt-5.4` 兜底),从缓存 catalog 取该 slug 的 `ModelInfo` 作 template。若该 slug 不在 catalog → 抛错(handler 返回 503)。
...
- `max_context_window`:与 `context_window` 始终相等;override 被 `applyOverride` 过滤,不独立生效(配置 `max_context_window` 不生效)
...
- codex 命令未装 / 执行失败 / 非零退出 → 503 `{error:{type:"server_error",message:"Failed to fetch codex bundled catalog: <reason>"}}`;`<reason>` 对 ZodError(schema 校验失败)用简短原因 `'codex catalog schema validation failed'`,不回显完整 ZodError JSON;其余错误保留 `err.message`
...
- catalog 缓存仅内存,重启刷新;codex 升级后需重启代理(或后续加刷新接口)。默认 `templateSlug` 动态选取,codex 升级移除某 slug 时默认配置不再硬性 503(除非 catalog 无任何 `supported_in_api=true` 条目)
...
- `context_window` / `max_context_window` = `limit.context` ?? 4 层 `contextWindow`(model > provider > 全局,默认 `200000`),两者始终相等;`max_context_window` override 字段被 `applyOverride` 过滤不生效(始终 = `context_window`);`context_window` 经 resolveContextWindow 生效(limit.context 优先);建议在 settings 配 `limit.context` 或 codex 覆盖以匹配真实模型上下文
